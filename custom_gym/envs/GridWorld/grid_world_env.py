...
class GridWorldEnv(gym.Env):
    ...
    def __init__(self, cfg: GridWorldEnvCfg, testing: bool = False):
        ...
        self.rewards = {
            "target_reached": lambda: 1 * cfg.reward_scales.has_reached_target if self._has_terminated() else 0,
        }
        
        assert cfg.sim.render_mode is None or cfg.sim.render_mode in self.metadata["render_modes"]
        self.render_mode = cfg.sim.render_mode
        self.render_fps = self.metadata["render_fps"] if cfg.sim.fps is None else cfg.sim.fps
    ...
    def _render_frame(self):
        ...
        pygame.draw.rect(
            ...
            (255, 0, 0),
            pygame.Rect(
                np.array([pix_square_size_width, pix_square_size_height]) * self._target_location,
        ...
        pygame.draw.circle(
            canvas,
            (0, 0, 255),
            (self._agent_location + 0.5) * np.array([pix_square_size_width, pix_square_size_height]),
