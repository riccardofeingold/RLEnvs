...
class GridWorldEnvCfg:
    ...
    observation_space = gym.spaces.Dict(
        {
            "agent": gym.spaces.Box(np.array([0, 0]), np.array([field_size[0]-1, field_size[1]-1]), shape=(2,), dtype=int),
            "target": gym.spaces.Box(np.array([0, 0]), np.array([field_size[0]-1, field_size[1]-1]), shape=(2,), dtype=int)
        }
    )
