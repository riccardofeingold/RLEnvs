...
class DroneWorldEnv(MujocoEnv, utils.EzPickle):
    ...
    def has_terminated(self) -> bool:
        ...
        if np.linalg.norm(contact_forces) > self.cfg.robot.max_contact_force_with_ground:
            return True
        return False
    ...
    def _get_reward(self, obs: np.ndarray[np.float64]) -> float:
        position_tracking_reward = gaussian_distribution(
            diff=np.linalg.norm(self.data.qpos[:3] - self.reference_position),
            mu=self.cfg.reward_scales.position_reference_tracking["mean"],
            sigma=self.cfg.reward_scales.position_reference_tracking["variance"]) * self.cfg.reward_scales.position_reference_tracking["scale"]

        slow_roll_pitch_rate_reward = gaussian_distribution(
    ...
    def step(self, action):
        ...
        truncated = True if self.elapsed_time >= self.cfg.max_episode_length_s else False
